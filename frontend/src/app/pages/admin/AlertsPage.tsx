 ...
 * Two tabs: Nhật ký cảnh báo / Thiết lập ngưỡng
 *
 * Alert logs load immediately on mount WITHOUT requiring a pond selection.
 * The ZonePondSelector acts as an optional filter for logs,
 * and is required only for threshold setup.
 * Backend handles role-based filtering via getUserPondIds().
 *
 * Metrics: Ánh sáng (light), Nhiệt độ (temperature), Mực nước (water_level)
 * Thresholds are strictly pond-level (target_type = 'pond').
 *
 * Realtime: Supabase Realtime subscription auto-refreshes the logs table on INSERT/UPDATE.
 */
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" disabled value={pondName || 'Chọn ao ở phần trên'} className={inputCls()} />
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="number" step="any" {...register('min_value', { required: 'Bắt buộc.' })} placeholder="0" className={inputCls(!!errors.min_value || !!minMaxErr)} />
            ...
            <div>
              ...
              <input type="number" step="any" {...register('max_value', { required: 'Bắt buộc.' })} placeholder="14" className={inputCls(!!errors.max_value || !!minMaxErr)} />
          ...
          <div className="sm:col-span-2 flex justify-end">
            <button type="submit" disabled={isSubmitting || !!minMaxErr}
                ...
                thresholds
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-gray-50/60 transition-colors">
                      <td className="px-4 py-3 text-sm text-gray-700 font-medium">{pondName || 'Ao đang chọn'}</td>
...
// ===== TAB 2: ALERT LOGS =====
// Realtime auto-refresh + pond/zone name display
...
const AlertLogsTab: React.FC<{ selectedPond: string; pondName: string; onUnreadChange?: (n: number) => void }> = ({ selectedPond, pondName, onUnreadChange }) => {
  ...
  const fetchLogs = useCallback(async (p: number) => {
    setIsLoading(true);
    try {
      const params: Parameters<typeof alertService.getAlertLogs>[0] = { page: p, limit: LIMIT };
      if (selectedPond) params.zoneId = selectedPond;
  ...
  useEffect(() => {
    const channel = supabase
      .channel('admin-alert-logs-refresh')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'alert_logs' }, () => {
        fetchLogs(pageRef.current);
        alertService.getUnreadCount().then((n) => onUnreadChange?.(n)).catch(() => null);
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'alert_logs' }, () => {
        fetchLogs(pageRef.current);
        alertService.getUnreadCount().then((n) => onUnreadChange?.(n)).catch(() => null);
      })
      .subscribe();
      ...
      <div className="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
        <h2 className="text-gray-800 text-sm font-semibold flex items-center gap-2">
          ...
          {selectedPond && pondName && (
            <span className="text-xs font-normal text-gray-400 ml-1">(lọc: {pondName})</span>
          )}
        </h2>
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <ZonePondSelector onPondSelect={setSelectedPond} onPondNameSelect={setSelectedPondName} />
        </div>
      </div>
